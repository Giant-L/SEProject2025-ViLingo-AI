...
|                                                                                          |
|  [文件存储] ->  -> [S1: 用户音频处理] -> [S2: 对比评分] -> [S3: 结果存储]  										 |
|      |               |                   |                    |                          |
|  (临时文件)        (Whisper)        (SentenceTransformer) (数据库/缓存)  									 |
|                                                                                          |
+------------------------------------------------------------------------------------------+
```
...
我们希望通过三个维度来对用户传入的录音文件进行打分，分别是：

* 语言守卫(The Language Guard)

  这一部分使用的是whisper模型首先转文本，方便后续的语义相似度分析使用，whisper可以保证用户输入的录音文件为英文，而不是其他语言。后续可以扩展更多功能。
* 
* 计算语义相似度
